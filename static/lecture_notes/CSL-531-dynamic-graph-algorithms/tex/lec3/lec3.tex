...
\end{algorithm}
\begin{remark}
    Since we use a DFS/BFS, it takes \(O(n+m)\) time for the algorithm to complete and each reachability query can be answered in \(O(1)\) time. The algorithm uses \(O(n)\) additional space for the array \(R\).
\end{remark}
