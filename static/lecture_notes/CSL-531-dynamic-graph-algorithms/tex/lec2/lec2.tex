...
\begin{enumerate}
    \item \emph{Upper Bound}. By \hyperref[sec:lemma1]{lemma 2.1}, we immediately obtain an upper bound on $T_d(n)$ given by 
        \[
...
\begin{enumerate}
    \item \emph{Upper Bound}. By \hyperref[sec:lemma1]{lemma 2.1}, we have
        \[
            \boxed{T_d(n) = \mathcal{O}\!\left(T_s(n)\right)}
        \]
    \item \emph{Lower Bound}. If each update on $G$ inserts a singleton vertex \emph{without} any edges, then assuming that no nontrivial recomputation is required for $\mathcal{P}$, we obtain the lower bound
        \[
            \boxed{T_d(n) = \Omega(1)}
        \]
    If instead, each update on $G$ adds a singleton vertex \emph{with} edges, then computing the lower bound for $T_d(n)$ in a similar fashion to that for \hyperref[sec:inc_edge_updates]{edge insertions}, we obtain
        \[
...
\]
Using \hyperref[sec:lemma1]{lemma 2.1}, we obtain
\[
